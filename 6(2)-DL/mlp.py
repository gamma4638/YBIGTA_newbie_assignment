...
class MultiLayerPerceptron(object):
    def __init__(self, nn_input_dim, nn_hdim1, nn_hdim2, nn_hdim3, nn_output_dim, init="random"):
        ...
        model 딕셔너리에 weight와 bias를 저장
        weight을 np.random.randn으로 초기화
        bias를 np.zeros로 초기화
    ...
    def forward_propagation(self, X):
        ...
        Returns:
            ...
            cache: (dict) Values needed to compute gradients (중간 결과 값)
        ...
        h2 = np.dot(z1, W2) + b2
        z2 = leakyrelu(h2)  # Leaky ReLU activation

        h3 = np.dot(z2, W3) + b3
        z3 = tanh(h3 + h1)  # Tanh activation with skip connection

        h4 = np.dot(z3, W4) + b4
        y_hat = sigmoid(h4)  # Sigmoid activation
        y_hat = y_hat.reshape(X.shape[0],) # 2차원 형태의 array를 1차원으로 변환
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        weight, bias, cache 입력과 실제 X, y를 사용해 gradient 계산 후 반환
        ...
        dh3 = dz3 * (1 - np.tanh(h3 + h1) ** 2)  # tanh derivative
        db3 = np.sum(dh3, axis=0)
        dW3 = z2.T @ dh3 + 2 * L2_norm * W3
        dz2 = dh3 @ W3.T

        # Skip connection
        dh1_skip = dz3 * (1 - np.tanh(h3 + h1) ** 2)

        # Second hidden layer (LeakyReLU)
        dh2 = dz2 * np.where(h2 > 0, 1, 0.01)  # LeakyReLU derivative
        db2 = np.sum(dh2, axis=0)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W2.T

        # First hidden layer (ReLU)
        dh1 = dz1 * (h1 > 0)  # ReLU derivative
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-4, L2_norm=0.0, epoch=50000, print_loss=True):
        """
        Descriptions:
        ...
        for it in range(epoch):
            ...
            loss = self.compute_loss(y_train, y_hat, L2_norm)
    ...
    def predict(self, X):
        """
        Descriptions:
            Predict binary class labels for the given input data X.
        
        Args:
            X: (numpy array) Input data (N, D)

        Returns:
            predictions: (numpy array) Predicted binary labels (0 or 1)
        """
...
def leakyrelu(x):
    ...
    Returns:
        numpy.ndarray: Leaky ReLU 함수가 적용된 출력 배열
    """
    ############################
